...
var overviewPage = {
    init: function() {
        ...
        // Announcement feed
        $.ajax({
            url:"https://ajax.googleapis.com/ajax/services/feed/load?v=2.0&q=http://www.appcoin.info/?feed=rss2",
            dataType: 'jsonp'
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" data-title="The address to send the payment to  (e.g. IDIleSqBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a INFO address (e.g. IDIleSqBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-right"> \
                <label for="narration[count]" class="recipient">Desc:</label> \
            ...
            <div class="flex-left"> \
                <label for="amount[count]" class="recipient">Number:</label> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="INFO"                    ' + (unit.type == 0 ? "selected" : "") + '>INFO</option> \
                    <option value="1" title="Milli-INFO (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mINFO</option> \
                    <option value="2" title="Micro-INFO (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;INFO</option> \
                    <option value="3" title="INFOshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>INFOshi</option> \
                </select> \
            </div> \
